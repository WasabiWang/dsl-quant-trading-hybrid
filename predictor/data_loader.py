...
数据源优先级:
1. 麦蕊API: 实时数据 + 最新财务指标（需配置 MAIRUI_LICENCE）
2. stockday_wd: 本地10年历史数据（Desktop/stockday_wd）
3. stock_daily CSV: 聚合格式备选
"""
import os
import glob
import pandas as pd
...
def load_stock_data(symbol: str, start_date: str = None, end_date: str = None) -> Optional[pd.DataFrame]:
    ...
    # === 优先级1: 麦蕊API（实时数据 + 最新财务指标）===
    try:
        from predictor.mairui_data import load_stock_data_mairui, _get_licence
        if _get_licence():
            df_mairui = load_stock_data_mairui(sym, start_date, end_date)
            if df_mairui is not None and len(df_mairui) > 0:
                # 补充本地数据中没有的字段
                ...
                if os.path.exists(csv_path):
                    try:
                        ...
                        df_local = df_local.rename(columns={k: v for k, v in col_map.items() if k in df_local.columns})
                        df_local = df_local.dropna(subset=['close'])
                        df_local.index.name = 'date'
                        
                        # 合并：用本地数据填充基本面字段，用麦蕊数据覆盖最新行情
                        local_cols = [c for c in df_local.columns if c not in df_mairui.columns or df_mairui[c].isna().all()]
                        if local_cols:
                            df_mairui = df_mairui.join(df_local[local_cols], how='left')
                            # 麦蕊数据覆盖本地同名字段
                            for col in df_mairui.columns:
                                if col in df_local.columns:
                                    mask = df_mairui[col].notna()
                                    if mask.any():
                                        pass  # 保留麦蕊数据
                    except Exception as e:
                        pass  # 合并失败不影响主流程
                
                print(f"  📡 麦蕊API数据: {len(df_mairui)}行")
                return df_mairui
    except ImportError:
        pass  # 麦蕊模块不可用，走本地数据
    except Exception as e:
        print(f"  ⚠️ 麦蕊API加载失败，回退本地数据: {e}")
