...
def execute_trades(trades: list, market: str = "A", enforce_prediction_freshness: bool = True):
    ...
    bs_active, max_positions = False, 3
    try:
        ...
        # v4.7.6 修复#4: 原默认 8, 与 config:trading.max_positions(=3) 脱节 → 改为 fail-closed 3
        max_positions = adaptive.get("trading", {}).get("max_positions", 3)
    except Exception as _cfg_err:
        print(f"⚠️ adaptive_params.yaml解析失败, 使用默认风控参数(bs_active=False, max_positions=3): {_cfg_err}")
