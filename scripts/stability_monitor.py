...
try:
    from core.stability_system import stability_system
    STABILITY_AVAILABLE = True
except ImportError:
    STABILITY_AVAILABLE = False
    print("⚠️ 稳定性系统未安装，请先运行稳定性集成")
...
def show_status():
    ...
    print(f"健康状态: {status['health']['health']}")
    ...
    for name, cb_status in status['circuit_breakers'].items():
        ...
        failures = cb_status.get('failure_count', 0)
    ...
    if 'error_rate' in metrics:
        ...
        if error_data.get('latest') is not None:
            error_rate = error_data['latest'] * 100
            print(f"  📊 错误率: {error_rate:.1f}%")
    
    if 'response_time' in metrics:
        rt_data = metrics['response_time']
        if rt_data.get('latest') is not None:
            print(f"  ⏱️ 响应时间: {rt_data['latest']:.2f}s")
    ...
    if alerts:
        ...
        for alert in alerts[-3:]:  # 最近3个告警
            old_state = alert.get('old_status', 'unknown')
            new_state = alert.get('new_status', 'unknown')
            timestamp = alert.get('timestamp', '')
            print(f"  ⚠️ {timestamp}: {old_state} → {new_state}")
...
def test_stability():
    ...
    try:
        ...
        from dsl_data_sdk_stable import get_price
        ...
        print("⚠️ 增强版数据SDK未找到，测试原始数据SDK...")
        try:
            from dsl_data_sdk import get_price
            result = get_price("000001.SZ")
            print(f"✅ 原始数据SDK测试成功: {result.get('name')}")
...
def check_system_health():
    ...
    status = stability_system.get_system_status()
    health_status = status['health']['health']
    ...
    if health_status == 'red' or open_circuits > 0:
        print("\n💡 建议:")
        if open_circuits > 0:
            print("  - 使用 'python3 scripts/stability_monitor.py reset <模块名>' 重置熔断器")
        print("  - 检查系统日志: tail -f dsl_errors.log")
        print("  - 查看详细状态: python3 scripts/stability_monitor.py status --detailed")
...
def main():
    ...
    reset_parser.add_argument('module', help='模块名称 (data_fetch, network, llm, strategy)')
